...
/*TODO
void BaseDriver::init_led_status()
{
    frame->interact(ID_SET_LED_STATUS);

    ROS_INFO_STREAM("[KUBOT]subscribe led_status topic on [" << bdg.led_status_topic << "]");

        led_status_pub = nh.advertise<kubot_msgs::RawLed>(bdg.led_status_topic,50);
        raw_led_status_msgs.set_ledNum = 1; 
        raw_led_status_msgs.set_led_brightness = 0;
        raw_led_status_msgs.set_led_speed = 0;
        raw_led_status_msgs.set_led_color = 0;
        raw_led_status_msgs.set_led_mode = 0;

        need_update_led_status = true; 
}
*/
...
/*TODO
void BaseDriver::led_status_callback(const kubot_msgs::RawLed& led_status)
{   
    ROS_INFO_STREAM("set_led_string:[ "<<raw_led_status_msgs.set_ledNum<<" ]to"<< raw_led_status_msgs.set_led_brightness<< "" <<raw_led_status_msgs.set_led_speed<<""<<raw_led_status_msgs.set_led_color<<""<< raw_led_status_msgs.set_led_mode<<);

    Data_holder::get()->led_status.ledNum = raw_led_status_msgs.set_ledNum;
    Data_holder::get()->led_status.led_brightness  = raw_led_status_msgs.set_led_brightness;
    Data_holder::get()->led_status.led_speed = raw_led_status_msgs.set_led_speed;
    Data_holder::get()->led_status.led_color = raw_led_status_msgs.set_led_color;
    Data_holder::get()->led_status.led_mode = raw_led_status_msgs.set_led_mode;
...
void BaseDriver::work_loop()
{
    ...
    while (ros::ok())
    {
        ...
        if (Data_holder::get()->parameter.imu_type == IMU_TYPE_MPU6050 || Data_holder::get()->parameter.imu_type == IMU_TYPE_GY85 || Data_holder::get()->parameter.imu_type == IMU_TYPE_GY87 || Data_holder::get()->parameter.imu_type == IMU_TYPE_MPU9250 || Data_holder::get()->parameter.imu_type == IMU_TYPE_BMI160)
        {
            update_imu();
        }

        //TODO    update_led_status();
...
{
    if (need_update_led_status)
    {
        ROS_INFO_STREAM("update_led_status");
        frame->interact(ID_SET_LED_STATUS);
        raw_led_status_msgs.header.stamp = ros::Time::now();
        raw_led_status_msgs.set_ledNum = Data_holder::get()->led_status.ledNum;
        raw_led_status_msgs.set_led_brightness = Data_holder::get()->led_status.led_brightness;
        raw_led_status_msgs.set_led_speed = Data_holder::get()->led_status.led_speed;
        raw_led_status_msgs.set_led_color = Data_holder::get()->led_status.led_color;
        raw_led_status_msgs.set_led_mode = Data_holder::get()->led_status.led_mode;

        need_update_led_status = false;
    }
}
*/
